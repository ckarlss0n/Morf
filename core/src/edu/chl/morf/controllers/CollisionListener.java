...
public class CollisionListener implements com.badlogic.gdx.physics.box2d.ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        CollisionData collisionDataA =new CollisionData(edu.chl.morf.userdata.CollisionType.OTHER);
        CollisionData collisionDataB =new CollisionData(edu.chl.morf.userdata.CollisionType.OTHER);
        ...
        if (contact.isTouching()) {
            ...
            if (collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_LEFT) {
                gameLogic.setGhostEmptyLeft(false);
            } else if (collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_LEFT) {
                gameLogic.setGhostEmptyLeft(false);
            } else if (collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_RIGHT) {
                gameLogic.setGhostEmptyRight(false);
            } else if (collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_RIGHT) {
                gameLogic.setGhostEmptyRight(false);
            }
            ...
            /*  When a block comes in contact with the ACTIVE_BLOCK_LEFT fixture the block is
                ...
                is currently active. */
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_LEFT){
                gameLogic.setActiveBody(fb.getBody(), ActiveBlockPosition.LEFT);
            } else if(collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_LEFT){
                gameLogic.setActiveBody(fa.getBody(), ActiveBlockPosition.LEFT);
            } else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_RIGHT){
                gameLogic.setActiveBody(fb.getBody(), ActiveBlockPosition.RIGHT);
            } else if(collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_RIGHT){
                gameLogic.setActiveBody(fa.getBody(), ActiveBlockPosition.RIGHT);
            } else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_LEFT){
                gameLogic.setActiveBody(fb.getBody(), ActiveBlockPosition.BOTTOM_LEFT);
            } else if(collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_LEFT){
                gameLogic.setActiveBody(fa.getBody(), ActiveBlockPosition.BOTTOM_LEFT);
            } else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_RIGHT){
                gameLogic.setActiveBody(fb.getBody(), ActiveBlockPosition.BOTTOM_RIGHT);
            } else if(collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_RIGHT){
            ...
            else if ((collisionTypeA == edu.chl.morf.userdata.CollisionType.SPIKE) && (collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM)) {
                gameLogic.killPlayer();
            } else if ((collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM) && (collisionTypeB == edu.chl.morf.userdata.CollisionType.SPIKE)) {
                gameLogic.killPlayer();
            }

            //Sets the jumping variable
            else if (collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM || collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM) {
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.WATER_SENSOR && collisionTypeB == edu.chl.morf.userdata.CollisionType.WATER){
                gameLogic.setWaterBottom(fa.getBody(), true);
                gameLogic.setWaterTop(fb.getBody(),true);
            }
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.WATER && collisionTypeB == edu.chl.morf.userdata.CollisionType.WATER_SENSOR){
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.FLOWER && collisionTypeB == edu.chl.morf.userdata.CollisionType.WATER_FLOWER_INTERSECTION){
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.WATER_FLOWER_INTERSECTION && collisionTypeB == edu.chl.morf.userdata.CollisionType.FLOWER){
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_CORE && collisionTypeB == edu.chl.morf.userdata.CollisionType.FLOWER){
                gameLogic.setPlayerInsideFlower(true);
            }else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.FLOWER && collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_CORE){
                gameLogic.setPlayerInsideFlower(true);
            }
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_CORE){
            ...
            else if(collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM_ICE || collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM_ICE){
    ...
    public void endContact(Contact contact) {
        ...
        else if ((collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_LEFT && collisionDataA.getNumOfContacts()==0) ||
                collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_LEFT && collisionDataB.getNumOfContacts()==0) {
            gameLogic.setActiveBody(null, ActiveBlockPosition.BOTTOM_LEFT);
        }
        else if((collisionTypeB == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_RIGHT && collisionDataB.getNumOfContacts()==0) ||
                collisionTypeA == edu.chl.morf.userdata.CollisionType.ACTIVE_BLOCK_BOTTOM_RIGHT && collisionDataA.getNumOfContacts()==0){
            gameLogic.setActiveBody(null, ActiveBlockPosition.BOTTOM_RIGHT);
        }

        //Sets jumping to true if GHOST_BOTTOM has no contacts.
        if(collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM) {
            if(collisionDataA.getNumOfContacts()==0) {
                gameLogic.setOnGround(false);
            }
        }
        if(collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM){
            if(collisionDataB.getNumOfContacts()==0) {
                gameLogic.setOnGround(false);
            }
        }
        ...
        else if (collisionTypeB == edu.chl.morf.userdata.CollisionType.WATER_SENSOR && collisionTypeA == edu.chl.morf.userdata.CollisionType.WATER){
            if(!(fb.getBody() == null) && !(fa.getBody() == null)){
                if(collisionDataB.getNumOfContacts() == 0){
        ...
        else if (collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_CORE || collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_CORE){
            if(collisionTypeA == edu.chl.morf.userdata.CollisionType.GHOST_CORE){
                if(collisionDataA.getNumOfContacts()==0){
        ...
        /*  GHOST_BOTTOM_ICE is a player fixture that can only register contact with water block in state ICE.
            When onIce is true, the player slides more. */
        else if (collisionTypeA == CollisionType.GHOST_BOTTOM_ICE || collisionTypeB == edu.chl.morf.userdata.CollisionType.GHOST_BOTTOM_ICE){
